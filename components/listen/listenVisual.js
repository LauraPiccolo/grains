...
export default function ListenVisual({ track, thisAudio, setPlayingStarted }) {
  const DEBUG = true;
  ...
  const start = () => {
    if(setPlayingStarted) setPlayingStarted(true)
  ...
  return (
    <div className="audio-player__visual">
      ...
      <div className="cover">
        <div className="t" style={{opacity:0}}>
          <span style={{fontSize: "0.5em"}}>click to play</span>
        </div>
      </div>
    </div>
  );
